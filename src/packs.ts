      ...
      pi: {
        promptSections: {
          acpTags: [
            ...
            `Refs are stable per message content and never reissued within a session; numbering restarts from m00001 only after native compaction or an upstream/model switch. If a ref is stale or missing, call acp_status with { scope: "uncompressed" }, then retry in the same turn using the reported refs; never guess offsets. Batch target ranges in one call.`,
