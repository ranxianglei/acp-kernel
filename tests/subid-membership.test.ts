...
test("computeProtectedRefs ignores covered messages re-projected under sub-ids (#234)", () => {
  ...
  assert.ok(
    !refs.has("m00001"),
    "covered message must not occupy a recent-window slot",
  );
});
...
test("collectBlockContent folds nested children across forms (#234)", () => {
  ...
  assert.ok(
    result.text.includes("a content"),
    "direct message rendered in full",
  );
  assert.ok(
    !result.text.includes("b content"),
    "nested-covered message stays folded",
  );
  assert.ok(
    result.text.includes("child summary"),
    "nested summary rendered in place",
  );
...
test("livelock guard fires when a range is fully covered across forms (#234)", () => {
  ...
  const messages = [
    msg("h_abc#r0", "already compressed content"),
    msg("h_post", "post"),
  ];
  assignAll(messages, state);
  const result = core.applyCompression({
...
test("block-boundary anchor resolves a base-id block over a sub-id view (#234)", () => {
  const state = createInitialState();
  state.blocks.push(
    makeBlock({ blockId: "b1", effectiveMessageIds: ["h_a", "h_b"] }),
  );
...
test("message ref in the older projection form still anchors to the visible message (#234)", () => {
  ...
  assert.equal(resolved.startIndex, 0);
  assert.deepEqual(
    resolved.snappedBoundaries,
    [],
    "no snap — the message is visible",
  );
});
