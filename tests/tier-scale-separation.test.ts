...
function makeMessages(count: number): CoreMessage[] {
  return Array.from({ length: count }, (_, i) =>
    textMessage(i % 2 === 0 ? "user" : "assistant", `m${i}`, `message ${i} content`),
  );
}
...
function grownState(blocks: CompressionBlock[], messageCount: number) {
  ...
  const first = core.processTurn({ messages, state: createInitialState(), config, tokenCount: 50_000 }).state;
...
test("T2 count boundary: 999 tier-1 blocks stay silent, 1000 fires (#383)", () => {
  ...
  const quiet = silent.core.processTurn({ messages: silent.messages, state: silent.state, config: silent.config, tokenCount: 60_000 });
  assert.equal(quiet.nudge.shouldInject, false, `reason: ${quiet.nudge.reason}`);
  assert.doesNotMatch(quiet.nudge.reason ?? "", /T2 distill ready/);

  const ready = grownState(microBlocks(1000, 1), 1031);
  const turn = ready.core.processTurn({ messages: ready.messages, state: ready.state, config: ready.config, tokenCount: 60_000 });
  assert.equal(turn.nudge.shouldInject, true, `reason: ${turn.nudge.reason}`);
  assert.equal(turn.nudge.tier, 2);
  assert.match(turn.nudge.reason ?? "", /1000 tier-1 blocks >= tier2Trigger 1000/);
});
...
test("T3 count boundary: 1999 tier-2 blocks stay silent, 2000 fires (#383)", () => {
  ...
  assert.match(turn.nudge.reason ?? "", /2000 tier-2 blocks >= tier3Trigger 2000/);
});
...
test("scale contract: default triggers encode 亿/几十亿 cumulative folded mass (#383)", () => {
  ...
  assert.ok(config.tiers.tier2Trigger * minFoldPerT1 >= 5_000_000, "T2 count floor must be >= 5M cumulative folded tokens");
  assert.ok(config.tiers.tier2Trigger * typicalFoldPerT1 >= 50_000_000, "T2 count typical scale must reach 亿-level (50M+)");
  assert.ok(config.tiers.tier3Trigger * 75_000 >= 150_000_000, "T3 count floor must be >= 1.5亿 cumulative (2000 x 75K distill mass)");
});
