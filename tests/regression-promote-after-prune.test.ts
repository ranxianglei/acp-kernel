...
import { prune, summaryMessageId } from "../src/prune.js";
...
// `acp_summary_bN` message. Each test FAILS against the pre-fix code.
...
function makeState(
  specs: { blockId: string; effectiveMessageIds: string[] }[],
  nextBlockId: number,
): CompressionState {
...
test("promote-after-prune: single T1 block promotes to T2 after its raws were replaced by the summary", () => {
  ...
  const turn = core.processTurn({ messages, state, config: cfg, tokenCount: 5000 });
  const visibleIds = turn.messages.map((m) => m.id);
  // Prune replaced the covered raws with the synthetic summary…
  assert.ok(visibleIds.includes(summaryMessageId("b2")), "summary should be visible");
  ...
  assert.deepEqual(applied.result.errors, [], `unexpected errors: ${applied.result.errors.join("; ")}`);
  ...
  for (const list of [newBlock.effectiveMessageIds, newBlock.directMessageIds]) {
    for (const id of list) {
      assert.ok(!id.startsWith("acp_summary_"), `synthetic id leaked into block: ${id}`);
    }
  }
  assert.equal(applied.state.blocks.find((b) => b.blockId === "b2")!.active, false);
});
...
test("promote-after-prune: two T1 blocks promote into one T2 consuming both", () => {
  ...
  const messages = Array.from({ length: 11 }, (_, i) =>
    msg(`raw-${i + 1}`, `m${i + 1} `.repeat(200), i % 2 === 0 ? "user" : "assistant"),
  );
  const state = makeState(
    [
      { blockId: "b2", effectiveMessageIds: ["raw-3", "raw-4"] },
  ...
  const turn = core.processTurn({ messages, state, config: cfg, tokenCount: 9000 });
  const applied = core.applyCompression({
  ...
  assert.deepEqual(newBlock.effectiveMessageIds.sort(), ["raw-3", "raw-4", "raw-5", "raw-6"]);
});
...
test("promote-after-prune: inclusive bN..bM selection consumes every active block in the span, including ones visible only via their summary", () => {
  ...
  const turn = core.processTurn({ messages, state, config: cfg, tokenCount: 8000 });
  const visibleIds = turn.messages.map((m) => m.id);
  ...
  assert.deepEqual(
    newBlock.effectiveMessageIds.sort(),
    ["raw-3", "raw-4", "raw-5", "raw-6", "raw-7", "raw-8"],
  );
  for (const id of ["b1", "b2", "b3"]) {
    assert.equal(applied.state.blocks.find((b) => b.blockId === id)!.active, false);
  }
});
...
test("syncBlocks: a block whose only visible representation is its rendered summary stays active", () => {
  ...
  const prunedView = [
    msg("raw-1", "u1 ".repeat(200)),
    msg("raw-2", "a1 ".repeat(200), "assistant"),
    { id: summaryMessageId("b2"), role: "system" as const, contentType: "text" as const, text: "[Compressed conversation section]\nsum" },
    msg("raw-5", "u5 ".repeat(200)),
  ];
...
test("prune idempotency: re-pruning an already-pruned view keeps the summary in place without duplicating it", () => {
  ...
  assert.equal(twice.indexOf(summaries[0]!), once.indexOf(once.find((m) => m.id === summaryMessageId("b2"))!));
});
